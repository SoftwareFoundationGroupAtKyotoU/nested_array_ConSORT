...
let rec print_declare oc var_locations fvs fun_num =
  ...
  (List.iter
    (fun (id,(pos,branch_trace)) ->
      ...
      fprintf formatter "(declare-fun o_%d_%s_%d%a () Real)\n" fun_num id pos pp_branch_trace branch_trace;
      (* 下限の係数を宣言 *)
       print_declare_c formatter fvs "l" id pos branch_trace fun_num;
      (* 所有権を表す下限の一次式の切片の宣言 
       [d + c1*x1 + ..., d' + c1'*x1 + ...] -> o のd
       d_(関数のシリアル番号)_l_(参照変数名)_(関数内での位置を表す整数)_(if or el) *)
       fprintf formatter "(declare-fun d_%d_l_%s_%d%a () Int)\n" fun_num id pos pp_branch_trace branch_trace;
       (* 上限の係数を宣言 *)
       print_declare_c formatter fvs "h" id pos branch_trace fun_num;
      (* 所有権を表す上限の一次式の切片の宣言 
       d_(関数のシリアル番号)_h_(参照変数名)_(関数内での位置を表す整数)_(if or el) *)
       fprintf formatter "(declare-fun d_%d_h_%s_%d%a () Int)\n" fun_num id pos pp_branch_trace branch_trace;
       ) var_locations);
(* 所有範囲の上限または下限の宣言 *)
and print_declare_c formatter fvs l_or_h id pos branch_trace fun_num =
  List.iter
    (fun fv ->
      ...
      fprintf formatter "(declare-fun c_%d_%s_%s_%s_%d%a () Int)\n"
    ...
    (fun (id,b_or_e,fun_num') ->
       if fun_num' = fun_num then
        ...
        (fprintf formatter "(declare-fun o_%d_%s_%s () Real)\n" fun_num id b_or_e;
         (* 下限の係数を宣言 *)
         print_declare_b_and_e_c formatter fvs "l" id b_or_e fun_num;
         (* 所有権を表す下限の一次式の切片の宣言 *)
         fprintf formatter "(declare-fun d_%d_l_%s_%s () Int)\n" fun_num id b_or_e;
         (* 上限の係数を宣言 *)
         print_declare_b_and_e_c formatter fvs "h" id b_or_e fun_num;
         (* 所有権を表す上限の一次式の切片の宣言 *)
...
and print_declare_b_and_e_c formatter fvs l_or_h id b_or_e fun_num =
  List.iter
    (fun fv ->
