...
let rec find_id (id,i) num ownerships =
  ...
  | Own (num',id1,i1,pos,Float f) :: rest when num = num' && id = id1 && i = i1 -> Own (num',id1,i1,pos,Float f) :: find_id (id,i) num rest
  (* CHigh:所有範囲の上限を表す一次式の係数　num':関数の順番番号？ id1:どの変数の係数となるか　id2:どの変数の所有権を表すか　i1:関数内の場所 i2:係数の数値？ *)
  | CHigh (num',id1,id2,i1,pos,Int i2) :: rest when num = num' && id = id2 && i = i1 -> CHigh (num',id1,id2,i1,pos,Int i2) :: find_id (id,i) num rest
  (* CHigh:所有範囲の下限を表す一次式の係数　num':関数の順番番号？ id1:どの変数の係数となるか　id2:どの変数の所有権を表すか　i1:関数内の場所 i2:係数の数値？ *)
  | CLow (num',id1,id2,i1,pos,Int i2) :: rest when num = num' && id = id2 && i = i1 -> CLow (num',id1,id2,i1,pos,Int i2) :: find_id (id,i) num rest
  (* DHigh:所有範囲の上限を表す一次式の切片　num':関数の順番番号？　id1:どの変数の所有権を表すか　i1:関数内の場所 i2:切片の数値？ *)
  | DHigh (num',id1,i1,pos,Int i2) :: rest when num = num' && id = id1 && i = i1 -> DHigh (num',id1,i1,pos,Int i2) :: find_id (id,i) num rest
  (* DHigh:所有範囲の下限を表す一次式の切片　num':関数の順番番号？ id1:どの変数の所有権を表すか　i1:関数内の場所 i2:切片の数値？ *)
  | DLow (num',id1,i1,pos,Int i2) :: rest when num = num' && id = id1 && i = i1 -> DLow (num',id1,i1,pos,Int i2) :: find_id (id,i) num rest
