...
type var_locations_ty = (id * (tyvar * branch list * simpleTy * id list)) list ref
(* 変数idとその変数の存在する場所 
(変数id, (変数の場所pos, branch_trace, simpleTy, fvs))のリスト*)
...
(* 所有範囲を共有するidの組のリスト *)
let immut_pair : (id * id) list ref = ref []
...
let rec lookup_pos id branch_trace env =
  ...
  | [] -> 
    raise Unbound
  | (x, (position, branch_trace', _, _)) :: left_env -> 
...
let new_id id position branch_trace simpleTy fvs =
  try
    ...
    if pos' = position then ()
      ...
      var_locations := (id, (position, branch_trace, simpleTy, fvs)) :: !var_locations
  with 
    Unbound -> var_locations := (id, (position, branch_trace, simpleTy, fvs)) :: !var_locations
...
var_locations((変数id, (関数内での場所, branch_trace, simpleTy, fvs))のリスト)から
...
let make_pre_bound_exp id h_or_l fun_num branch_trace depth =
...
let make_idx_bound_smtlib id idx fvs fun_num branch_trace depth =
  And(Leq(Id idx, make_bound_exp fvs id "_h" fun_num branch_trace depth), 
    Geq(Id idx, make_bound_exp fvs id "_l" fun_num branch_trace depth))

let make_idx_pre_bound_smtlib id idx fun_num branch_trace depth =
  And(Leq(Id idx, make_pre_bound_exp id "_h" fun_num branch_trace depth), 
    Geq(Id idx, make_pre_bound_exp id "_l" fun_num branch_trace depth))

let make_idx_bound_smtlib_be id idx fvs fun_num b_or_e depth =
  And(Leq(Id idx, make_bound_exp_be fvs id "_h" fun_num b_or_e depth), 
    Geq(Id idx, make_bound_exp_be fvs id "_l" fun_num b_or_e depth))
...
let make_if_smtlib id fvs fun_num branch_trace depth =
  let rec make_if_smtlib_sub fvs depth =
    if depth <= 0 then [], [], []
      ...
      [same_bound_branch fvs id "_h" fun_num branch_trace (Then :: branch_trace) depth;
        same_bound_branch fvs id "_l" fun_num branch_trace (Then :: branch_trace) depth;],
      [same_bound_branch fvs id "_h" fun_num branch_trace (Else :: branch_trace) depth;
        same_bound_branch fvs id "_l" fun_num branch_trace (Else :: branch_trace) depth;] in
...
let make_post_if_smtlib fvs id fun_num branch_trace depth branch =
      ...
      [Leq(make_own_var id fun_num branch_trace depth, make_own_var id fun_num branch_trace' depth)]
       ...
       ( List.map
        (fun x -> 
          ...
          Or(Gt(make_bound_exp fvs id "_l" fun_num branch_trace (depth - 1),
            make_bound_exp fvs id "_h" fun_num branch_trace (depth - 1)), x)))
          sl_own),
      [Geq(make_bound_exp fvs id "_l" fun_num branch_trace depth,
       make_bound_exp fvs id "_l" fun_num branch_trace' depth);
      Leq(make_bound_exp fvs id "_h" fun_num branch_trace depth, 
        make_bound_exp fvs id "_h" fun_num branch_trace' depth);]
        @ (List.map
      (fun x -> 
        Or(Eq(make_own_var id fun_num branch_trace (depth - 1), Id "0."), 
        Or(Gt(make_bound_exp fvs id "_l" fun_num branch_trace (depth - 1),
          make_bound_exp fvs id "_h" fun_num branch_trace (depth - 1)),
  ...
  (fun x -> 
    ...
    Or(Gt(make_bound_exp fvs id "_l" fun_num branch_trace depth,
      make_bound_exp fvs id "_h" fun_num branch_trace depth), x)))
  (sl_own @ sl_range)
...
let make_mkarray_smtlib fvs id fun_num branch_trace depth exp =
  ...
  (fun fv -> 
    let var_name_l = asprintf "c_%d_%s_%s_%s_%d%a_%d" fun_num "_l" fv id id_pos pp_branch_trace branch_trace depth in
    let var_name_h = asprintf "c_%d_%s_%s_%s_%d%a_%d" fun_num "_h" fv id id_pos pp_branch_trace branch_trace depth in
    try 
    ...
    fvs in
  let intercept_sl = 
    let var_name_l = asprintf "d_%d_%s_%s_%d%a_%d" fun_num "_l" id id_pos pp_branch_trace branch_trace depth in
    let var_name_h = asprintf "d_%d_%s_%s_%d%a_%d" fun_num "_h" id id_pos pp_branch_trace branch_trace depth in
    try 
...
let make_annotated_mkarray_smtlib fvs id fun_num branch_trace depth ftype =
...
let make_letAddPtr_smtlib fvs fun_num branch_trace id1 id2 sl depth =
  ...
  let idx_bound id idx fvs depth x = 
    ...
    x) in
  let idx_pre_bound id idx depth x = 
    Imply(make_idx_pre_bound_smtlib id idx fun_num branch_trace depth,
    x) in
  ...
  let rec make_letAppPtr_smtlib_range fvs1 fvs2 depth =
    if depth <= 0 then True, True
      ...
      let sl1, sl2 = 
      And(Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
            make_bound_exp fvs1 id1 "_l" fun_num branch_trace depth),
          Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, 
            make_bound_exp fvs1 id1 "_h" fun_num branch_trace depth)),
      And(Eq(make_bound_exp fvs1 id1 "_l" fun_num branch_trace depth,
            make_bound_exp fvs2 id2 "_l" fun_num branch_trace depth),
          Eq(make_bound_exp fvs1 id1 "_h" fun_num branch_trace depth,
            make_bound_exp fvs2 id2 "_h" fun_num branch_trace depth))  in
      let sl3, sl4 = make_letAppPtr_smtlib_range fvs1' fvs2' (depth-1) in
      And(sl1, 
        idx_pre_bound id2 idx2 depth
  ...
  let sl1 = 
      ...
      And(Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
      make_bound_exp fvs id2 "_l" fun_num branch_trace depth),
      And(Eq(make_bound_exp fvs id1 "_l" fun_num branch_trace depth, Id "0"),
      And(Eq(make_bound_exp fvs id2 "_h" fun_num branch_trace depth,
      Sub(sl, Id "1")),
      Eq(make_bound_exp fvs id1 "_h" fun_num branch_trace depth,
      Sub(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, sl)))))))) in
  (* 最上位の参照の所有範囲の共有の際の制約 *)
  (* let sl2 = 
    ...
    And(Eq(make_pre_bound_exp fvs id2 "_l" fun_num branch_trace depth,
      Add(make_bound_exp fvs id1 "_l" fun_num branch_trace depth, sl)),
    And(Eq(make_pre_bound_exp fvs id2 "_l" fun_num branch_trace depth,
      make_bound_exp fvs id2 "_l" fun_num branch_trace depth),
    And(Eq(make_pre_bound_exp fvs id2 "_h" fun_num branch_trace depth, 
      Add(make_bound_exp fvs id1 "_h" fun_num branch_trace depth, sl)),
    Eq(make_pre_bound_exp fvs id2 "_h" fun_num branch_trace depth,
      make_bound_exp fvs id2 "_h" fun_num branch_trace depth))))) in *)
...
let make_letAddPtr_smtlib_heuristic fvs fun_num branch_trace id1 id2 depth =
  ...
  let idx_pre_bound id idx depth = make_idx_pre_bound_smtlib id idx fun_num branch_trace depth in
  (* 所有権の値の制約 *)
  ...
  let rec same_range id2_0 id2 fvs2_0 fvs2 depth =
    if depth <= 0 then []
      ...
      let sl1 = 
        ...
        [And(Eq(make_bound_exp fvs2 id2 "_l" fun_num branch_trace depth,
          make_pre_bound_exp id2_0 "_l" fun_num branch_trace depth),
        Eq(make_bound_exp fvs2 id2 "_h" fun_num branch_trace depth, 
          make_pre_bound_exp id2_0 "_h" fun_num branch_trace depth))] in
      let sl2 = same_range id2_0 id2 fvs2'_0 fvs2' (depth-1) in
      sl1 @
      List.map
      (fun x -> 
        Imply(idx_pre_bound id2_0 idx2_0 depth,
  ...
  let sl1 = 
      [
        Eq(make_bound_exp fvs id1 "_l" fun_num branch_trace depth, Id "0");
        Eq(make_bound_exp fvs id1 "_h" fun_num branch_trace depth,
          make_pre_bound_exp id2_non0 "_h" fun_num branch_trace depth);
        Eq(make_bound_exp fvs id2 "_l" fun_num branch_trace depth, Id "0");
        Eq(make_bound_exp fvs id2 "_h" fun_num branch_trace depth, Id "0");
      ] in
  ...
  List.map
  (fun x ->
    Imply(idx_pre_bound id2_non0 idx2_non0 depth, Imply(idx_bound id1 idx1 fvs depth,
      Imply(Eq(Id idx2_non0, Id idx1), x))))
  (same_range id2_non0 id1 fvs2_non0 fvs1 (depth-1))
with | Unbound -> raise ConstrError

(* let immut id1 = id2(ref) + n in ... *)
let make_letImmutAddPtr_smtlib fvs fun_num branch_trace id1 id2 sl depth =
  (* 所有範囲を分割した場合 *)
  let idx_bound id idx fvs depth x = 
    Imply(make_idx_bound_smtlib id idx fvs fun_num branch_trace depth,
    x) in
  let idx_pre_bound id idx depth x = 
    ...
    x) in
    (* 所有範囲が共有されている *)
  let rec make_letImmutAppPtr_smtlib_share depth =
    (if depth <= 0 then True
    else
      let sl1 = Eq(make_pre_own_var id2 fun_num branch_trace depth,
      Add(make_own_var id1 fun_num branch_trace depth, make_own_var id2 fun_num branch_trace depth)) in
      let sl2 = make_letImmutAppPtr_smtlib_share (depth-1) in
      And(sl1, sl2)) in
  (* 新しくできたid1もid2も元のid2の所有範囲を引き継ぐ *)
  let rec make_letImmutAppPtr_smtlib_range fvs1 fvs2 depth =
    if depth <= 0 then True, True
    else
      let idx1 = make_idx_id fun_num id1 depth in
      let fvs1' = idx1::fvs in
      ...
      And(Eq(make_bound_exp fvs1 id1 "_l" fun_num branch_trace depth,
          ...
          Eq(make_bound_exp fvs1 id1 "_h" fun_num branch_trace depth,
            make_bound_exp fvs2 id2 "_h" fun_num branch_trace depth))  in
      let sl3, sl4 = make_letImmutAppPtr_smtlib_range fvs1' fvs2' (depth-1) in
      And(sl1, 
        idx_pre_bound id2 idx2 depth
          (idx_bound id1 idx1 fvs1 depth 
            (Imply(Eq(Id idx1, Id idx2), sl3)))),
  ...
  let sl2 = 
    Ands[
      Eq(make_pre_own_var id2 fun_num branch_trace depth, 
        Add(make_own_var id1 fun_num branch_trace depth,
        make_own_var id2 fun_num branch_trace depth));
      Gt(make_own_var id1 fun_num branch_trace depth, Id "0");
      Gt(make_own_var id2 fun_num branch_trace depth, Id "0");
      Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
        Add(make_bound_exp fvs id1 "_l" fun_num branch_trace depth, sl));
      Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
        make_bound_exp fvs id2 "_l" fun_num branch_trace depth);
      Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, 
  ...
  let sl3, sl4 = make_letImmutAppPtr_smtlib_range fvs1 fvs2 (depth-1) in
  [
    And(sl2, make_letImmutAppPtr_smtlib_share (depth-1));
    idx_pre_bound id2 idx2 depth
      (idx_bound id1 idx1 fvs depth
        (Imply(Eq(Id idx1, Add(Id idx2, sl)), sl3)));
    idx_bound id1 idx1 fvs depth
...
let make_assignRef_smtlib fvs fun_num branch_trace id1 id2 depth =
  ...
  let rec first_element fvs1_0 fvs2 depth =
    ...
    if depth <= 0 then []
      ...
      [Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth, make_bound_exp fvs1_0 id1_0 "_l" fun_num branch_trace depth);
      Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, make_bound_exp fvs1_0 id1_0 "_h" fun_num branch_trace depth)]
      @ 
      List.map 
        (fun x -> 
          idx_pre_bound id2 idx2 depth 
  ...
  let rec not_first_element fvs1 fvs1_non0 depth =
    ...
    if depth <= 0 then []
      ...
      [Eq(make_pre_bound_exp id1 "_l" fun_num branch_trace depth, 
        make_bound_exp fvs1_non0 id1_non0 "_l" fun_num branch_trace depth);
      Eq(make_pre_bound_exp id1 "_h" fun_num branch_trace depth, 
        make_bound_exp fvs1_non0 id1_non0 "_h" fun_num branch_trace depth)]
      @ 
      List.map 
        (fun x -> 
          idx_bound id1_non0 idx1_non0 fvs1_non0 depth 
           (idx_pre_bound id1 idx1 depth 
...
let make_letDeref_smtlib fvs fun_num branch_trace id1 id2 depth =
  ...
  let rec make_letDeref_smtlib_same_range id1 id2 fvs1 fvs2 depth =
    if depth <= 0 then []
      ...
      let sl1 = 
        [
        Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth, make_bound_exp fvs1 id1 "_l" fun_num branch_trace depth);
        Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, make_bound_exp fvs1 id1 "_h" fun_num branch_trace depth);
        ] in
...
let make_aliasDeref_smtlib fvs fun_num branch_trace id1 id2 depth =
  ...
  let rec make_aliasDeref_smtlib_same_range_pre fvs1 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      let sl1 = 
        [
          Eq(make_bound_exp fvs2_non0 id2_non0 "_l" fun_num branch_trace depth, make_pre_bound_exp id1 "_l" fun_num branch_trace depth);
          Eq(make_bound_exp fvs2_non0 id2_non0 "_h" fun_num branch_trace depth, make_pre_bound_exp id1 "_h" fun_num branch_trace depth);
        ] in
  ...
  let rec make_aliasDeref_smtlib_same_range_post fvs2 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      let sl1 = 
        [
        Eq(make_bound_exp fvs2_non0 id2_non0 "_l" fun_num branch_trace depth, make_bound_exp fvs2 id2 "_l" fun_num branch_trace depth);
        Eq(make_bound_exp fvs2_non0 id2_non0 "_h" fun_num branch_trace depth, make_bound_exp fvs2 id2 "_h" fun_num branch_trace depth);
        ] in
  ...
  let fvs2 = idx2::fvs in
  let idx2_non0 = make_idx_id fun_num id2_non0 depth in
  let fvs2_non0 = idx2_non0::(lookup_fvs id2_non0 branch_trace !var_locations depth fun_num) in
  common (depth-1) 
  @ (List.map 
...
let rec constr_to_smtlib fvs fun_num funnames_numberings branch_trace ty_env c =
  ...
  | CIf (e,cs1,cs2,pos) -> 
    ...
    List.iter (fun id -> 
      ...
      new_id id pos (Then :: branch_trace) simpleTy fvs; 
      new_id id pos (Else :: branch_trace) simpleTy fvs) ids_pre;
    let constraints_pre = 
      (* 制約のリスト[Eq(...); Eq(...); ..., Eq(...)]を作る *)
    ...
    List.iter (fun id -> 
      (* Format.printf "5\n"; flush stdout;  *)
      let simpleTy = lookup id ty_env in
      new_id id (pos+1) branch_trace simpleTy fvs) ids_post;
  ...
  | CLetAddPtr (id1,id2,e,pos) ->  (* Corresponds to the example on p.15, let x = y + num in ... *)
    ...
    new_id id1 pos branch_trace simpleTy fvs; 
    new_id id2 pos branch_trace simpleTy fvs;
    (* numをsmtlibの読める形に変形 *)
    ...
    if contains_element eq0_list id2 && sl1 = Id "1" then
      ...
      new_id (id2^"_eq0") pos branch_trace simpleTy fvs;
      new_id (id2^"_non0") pos branch_trace simpleTy fvs;
  ...
  | CLetImmutAddPtr (id1,id2,e,pos) ->  (* let immut x = y + num in ... *)
  (* x,yに対応するvar_locationsを追加 *)
    let simpleTy = lookup id2 ty_env in
    new_id id1 pos branch_trace simpleTy fvs; 
    new_id id2 pos branch_trace simpleTy fvs;
    immut_pair := (id1,id2) :: !immut_pair;
    let sl1 = exp_to_smtlib e in
    let depth = ref_depth (lookup id2 ty_env) in
    make_letImmutAddPtr_smtlib fvs fun_num branch_trace id1 id2 sl1 depth  
  | CLetUndet(id, cs) ->
    List.concat (List.map (constr_to_smtlib (id::fvs) fun_num funnames_numberings branch_trace ty_env) cs) 
  | CMkArray (id,e, ftype,pos) -> 
    (* let x = alloc e in ... *)
    (* xに対応するvar_locationsを追加 *)
    new_id id pos branch_trace (ftype_to_simplety ftype) fvs;
    (* 
    xの所有権は1;
    xの所有範囲の下限は0;
    xの所有範囲の上限はe-1 *)
    let depth = ref_depth (lookup id ty_env) in
    (match ftype with
    | FTRef(_, ENull, ENull, _) ->
      make_mkarray_smtlib fvs id fun_num branch_trace depth e
    | FTRef _ ->
      make_annotated_mkarray_smtlib fvs id fun_num branch_trace depth ftype
    | FTInt _ ->
      raise (Error "mkarray annotation error")  )
  | CAssignInt (id,pos) -> 
    (* x := num; ... *)
    (* 
    xの所有権は1;
    ...
    [Eq(make_pre_own_var id fun_num branch_trace 1, Id "1");
     Leq(make_pre_bound_exp id "_l" fun_num branch_trace 1, Id "0"); 
     Geq(make_pre_bound_exp id "_h" fun_num branch_trace 1, Id "0");
     Eq(make_pre_own_var id fun_num branch_trace 1, make_own_var id fun_num branch_trace 1);
     Eq(make_pre_bound_exp id "_l" fun_num branch_trace 1, make_bound_exp fvs id "_l" fun_num branch_trace 1);
     Eq(make_pre_bound_exp id "_h" fun_num branch_trace 1, make_bound_exp fvs id "_h" fun_num branch_trace 1)]
  | CAssignRef (id1, id2, pos) -> 
    ...
    new_id id1 pos branch_trace id1_simpleTy fvs; 
    new_id id1_eq0 pos branch_trace id1_simpleTy fvs; 
    new_id id1_non0 pos branch_trace id1_simpleTy fvs;
    new_id id2 pos branch_trace id2_simpleTy fvs; 
    ...
    let sl1 = [Eq(make_pre_own_var id1 fun_num branch_trace id1_depth, Id "1.");
     Eq(make_pre_bound_exp id1 "_l" fun_num branch_trace id1_depth, Id "0"); 
     Geq(make_pre_bound_exp id1 "_h" fun_num branch_trace id1_depth, Id "0");
     ...
     Eq(make_bound_exp fvs id1_eq0 "_l" fun_num branch_trace id1_depth, Id "0");
     Eq(make_bound_exp fvs id1_eq0 "_h" fun_num branch_trace id1_depth, Id "0");
     Eq(make_bound_exp fvs id1_non0 "_l" fun_num branch_trace id1_depth, Id "0");
     Eq(make_bound_exp fvs id1_non0 "_h" fun_num branch_trace id1_depth, 
      Sub(make_pre_bound_exp id1 "_h" fun_num branch_trace id1_depth, Id"1"))] in
  ...
  | CAliasAddPtr (id1,id2,e,pos) -> (* alias(id1 = id2 + num); ... *)
    ...
    let rec gather_own depth =
      if depth <= 0 then True
        ...
        (* 再帰処理 *)
        let sl3 = gather_own (depth-1) in
        And(sl1, And(sl2, sl3)) in
    let rec share_own depth =
      if depth <= 0 then True
      else
        (* id1とid2の所有権を合算 *)
        let sl1 = 
            Eq(Add(make_pre_own_var id1 fun_num branch_trace depth,
              make_pre_own_var id2 fun_num branch_trace depth),
              make_own_var id2 fun_num branch_trace depth) in
        (* id1の所有権は0になる *)
        let sl2 = 
    ...
    let make_aliasdAddPtr_smtlib_gather depth = 
      let rec same_range_pre fvs1 fvs2 depth =
        if depth <= 0 then True
          ...
          (* id1とid2の元の所有範囲は等しい *)
          let sl1 = 
            And(Eq(make_pre_bound_exp id1 "_l" fun_num branch_trace depth,
              make_pre_bound_exp id2 "_l" fun_num branch_trace depth),
            Eq(make_pre_bound_exp id1 "_h" fun_num branch_trace depth,
            make_pre_bound_exp id2 "_h" fun_num branch_trace depth)) in
    ...
    then 
      ...
      let rec make_aliasdAddPtr_smtlib_gather depth =
        ...
        (* id1とid2の小さい方の所有権を採用 *)
          ...
          let sl2 = 
            And(Eq(make_pre_bound_exp id2_0 "_l" fun_num branch_trace depth,
              make_bound_exp fvs id2 "_l" fun_num branch_trace depth),
            And(Eq(make_pre_bound_exp id2_0 "_h" fun_num branch_trace depth,
              make_bound_exp fvs id2 "_h" fun_num branch_trace depth),
            Eq(make_own_var id1 fun_num branch_trace depth, Id "0"))) in
            (* id1とid2の元の所有範囲は等しい *)
        let sl3 = 
          And(Eq(make_pre_bound_exp id1 "_l" fun_num branch_trace depth,
    ...
    (* let rec make_aliasdAddPtr_smtlib_no_change depth =
      if depth <= 0 then True
        ...
        (* 所有範囲は変化しない *)
        let sl2 =
          And(Eq(make_pre_bound_exp fvs id2_0 "_l" fun_num branch_trace depth,
              make_bound_exp fvs id2 "_l" fun_num branch_trace depth),
          And(Eq(make_pre_bound_exp fvs id2_0 "_h" fun_num branch_trace depth,
            make_bound_exp fvs id2 "_h" fun_num branch_trace depth),
          Eq(make_own_var id1 fun_num branch_trace depth, Id "0"))) in
          (* 再帰処理 *)
        ...
        And(Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth, Id "0"),
        And(Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, Sub(sl, Id "1")),
        ...
        And(Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
          make_bound_exp fvs id2 "_l" fun_num branch_trace depth),
        Eq(Add(make_pre_bound_exp id2 "_h" fun_num branch_trace depth, 
          Add(make_pre_bound_exp id1 "_h" fun_num branch_trace depth, Id "1")),
          make_bound_exp fvs id2 "_h" fun_num branch_trace depth)))))) in
        (* id1の元の所有権が0の場合 *)
      let sl2 = 
        And(Eq(make_pre_own_var id2 fun_num branch_trace depth,
    ...
    (* id1とid2の元の所有範囲が共有されている場合 *)
      let sl1 = 
        And(Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace depth,
          Add(make_pre_bound_exp id1 "_l" fun_num branch_trace depth, sl)),
        And(Eq(make_pre_bound_exp id2 "_h" fun_num branch_trace depth,
          Add(make_pre_bound_exp id1 "_h" fun_num branch_trace depth, sl)),
      ...
      (* let sl3 = 
        ...
        Eq(make_pre_bound_exp fvs id2 "_h" fun_num branch_trace depth,
        make_bound_exp fvs id2 "_h" fun_num branch_trace depth))) in *)
        if List.mem (id1, id2) !immut_pair then 
          [sl1;
          share_own (depth-1);
  ...
  | CLetDeref (id1, id2, pos) -> 
    ...
    new_id id1 pos branch_trace id1_simpleTy fvs; 
    new_id id2 pos branch_trace id2_simpleTy fvs;
    new_id id2_0 pos branch_trace id2_simpleTy fvs;
    new_id id2_non0 pos branch_trace id2_simpleTy fvs;
    let id1_depth = ref_depth id1_simpleTy in
    ...
    let sl1 = [Gt(make_pre_own_var id2 fun_num branch_trace id2_depth, Id "0");
     Eq(make_pre_bound_exp id2 "_l" fun_num branch_trace id2_depth, Id "0"); 
     Geq(make_pre_bound_exp id2 "_h" fun_num branch_trace id2_depth, Id "0");
     ...
     Eq(make_bound_exp fvs id2_non0 "_h" fun_num branch_trace id2_depth,
        Sub(make_pre_bound_exp id2 "_h" fun_num branch_trace id2_depth, Id "1"))] in
  ...
  | CApp (fun_name,args,pos) -> 
    ...
    (* 関数定義の制約生成 *)
    let subst_param_before_eval param arg depth =
      let rec subst_param_before_eval_sub fvs param arg depth = 
      if depth <= 0 then []
      else
        match param, arg with
        (* x | () ref *)
        | (RawId id_param, FTRef (_,ENull,ENull,_)), AId id ->
          ...
          let rec same_range fvs fvs_param depth =
            (* 所有範囲の下限を表すデータ構造 *)
            let sll = make_bound_exp_be fvs' id_param "_l"  num "b" depth in
            (* 所有範囲の上限を表すデータ構造 *)
            let slh = make_bound_exp_be fvs' id_param "_h"  num "b" depth in
            let idx = make_idx_id fun_num id depth in
            let fvs' = idx::fvs in
            ...
            if depth <= 0 then True
              ...
              let sl1 = And(Leq(make_bound_exp fvs id "_l" fun_num branch_trace depth, smtlib_subst subst sll),
              Geq(make_bound_exp fvs id "_h" fun_num branch_trace depth, smtlib_subst subst slh)) in
        ...
        | (RawId id_param, FTRef (ftype,el,eh,f)), AId id -> 
          ...
          if depth <= 1 then
            ...
            And(geq "_h" fvs,
            Imply(make_idx_bound_smtlib id idx fvs fun_num branch_trace depth,
            List.hd (subst_param_before_eval_sub (idx::fvs) (RawId id_param, ftype) arg (depth-1)))))))]
          (* 整数の時は何もしない *)
        | _, AExp _ -> []
    ...
    let rec subst_param_after_eval fvs ftid_ft arg depth = 
      if depth <= 0 then []
        ...
        | (RawId id_param, FTRef (_,ENull,ENull,_)), AId id ->
          ...
          let rec same_range fvs fvs_param depth =
            (* 所有範囲の下限を表すデータ構造 *)
            let sll = make_bound_exp_be fvs' id_param "_l"  num "e" depth in
            (* 所有範囲の上限を表すデータ構造 *)
            let slh = make_bound_exp_be fvs' id_param "_h"  num "e" depth in
            let idx = make_idx_id fun_num id depth in
            let fvs' = idx::fvs in
            let idx_param = make_idx_id num id_param depth in
            let fvs_param' = idx_param::fvs_param in
            if depth <= 0 then True
              ...
              let sl1 = And(Eq(make_bound_exp fvs id "_l" fun_num branch_trace depth, smtlib_subst subst sll),
                Eq(make_bound_exp fvs id "_h" fun_num branch_trace depth, smtlib_subst subst slh)) in
              let sl2 = same_range fvs' fvs_param' (depth-1) in
              And(sl1, 
                Imply(And(Eq(Id idx, Id idx_param),
                  And(make_idx_bound_smtlib id idx fvs fun_num branch_trace depth,
  ...
  | CAliasDeref (id1, id2, pos) -> 
      ...
      let sl1 = 
        ...
        Eq(make_own_var id2_0 fun_num branch_trace id2_depth,
          make_own_var id2_non0 fun_num branch_trace id2_depth);
        Eq(make_bound_exp fvs id2 "_l" fun_num branch_trace id2_depth, Id "0");
        Eq(make_bound_exp fvs id2 "_h" fun_num branch_trace id2_depth, 
          Add(make_bound_exp (lookup_fvs id2_non0 branch_trace !var_locations id2_depth fun_num) id2_non0 "_h" fun_num branch_trace id2_depth, Id "1"));
        ] in
...
let fun_constrs_to_smtlib funname_constrs fun_num funnames_numberings =
  ...
  (* 関数引数名をvar_locations(変数id, (変数の位置, branch_trace, 単純型, fvs))のリストに代入 *)
  var_locations := 
  List.map (fun id -> 
    let simple_ty = lookup id ty_env in
    (id, (0, [], simple_ty, fvs))) ref_ids;
  ...
  let ref_id_before_eval_to_smtlibs id = 
    ...
    | FTRef (ftype', exp_low, exp_high, own) ->
      ...
      | ENull ->
        ...
        [Eq(make_bound_exp fvs id "_l" fun_num [] depth, make_bound_exp_be fvs id "_l" fun_num "b" depth);
        Eq(make_bound_exp fvs id "_h" fun_num [] depth, make_bound_exp_be fvs id "_h" fun_num "b" depth);
        ] in
  ...
  let ref_id_after_eval_to_smtlibs id = 
    ...
    | ENull ->
      ...
      [Or(Eq(make_own_var_be id fun_num "e" depth, Id "0."),
       ...
       And(Geq(make_bound_exp_be fvs id "_l" fun_num "e" depth, make_bound_exp fvs id "_l" fun_num [] depth),
           Leq(make_bound_exp_be fvs id "_h" fun_num "e" depth, make_bound_exp fvs id "_h" fun_num [] depth))))] 
      in
  ...
  let make_scope_limit_smtlib (id, (pos,branch_trace, simpleTy, _)) =
    let depth = ref_depth simpleTy in
