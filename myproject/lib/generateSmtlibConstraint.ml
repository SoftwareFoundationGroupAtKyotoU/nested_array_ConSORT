...
let make_assignRef_smtlib fvs fun_num branch_trace id1 id2 depth =
  ...
  let rec first_element fvs1_0 fvs2 depth =
    ...
    if depth <= 0 then []
      ...
      List.map 
        (fun x -> 
          idx_bound id2 idx2 fvs2 depth (idx_bound id1_0 idx1_0 fvs1_0 depth x))
          (List.map 
  ...
  let rec not_first_element fvs1 fvs1_non0 depth =
    ...
    if depth <= 0 then []
      ...
      List.map 
        (fun x -> 
          idx_bound id1_non0 idx1_non0 fvs1_non0 depth (idx_bound id1 idx1 fvs1 depth x))
          (List.map 
...
let make_aliasDeref_smtlib fvs fvs_pre fun_num branch_trace id1 id2 depth =
  ...
  let rec make_aliasDeref_smtlib_same_range_pre fvs1 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      sl1 @ 
      List.map
        (fun x -> And(idx_bound id1 idx1 fvs1 depth x, idx_bound id2_non0 idx2_non0 fvs2_non0 depth x)) 
  ...
  let rec make_aliasDeref_smtlib_same_range_post fvs2 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      sl1 @ 
      List.map
        (fun x -> idx_bound id2 idx2 fvs2 depth (idx_bound id2_non0 idx2_non0 fvs2_non0 depth x))
...
let rec constr_to_smtlib fvs fun_num funnames_numberings branch_trace ty_env c =
    ...
    let make_aliasdAddPtr_smtlib_gather depth =
      ...
      let rec same_range_post fvs2 fvs2_pre depth =
        if depth <= 0 then True
          ...
          And(sl1, 
           idx_bound id2 idx2 fvs2 depth
            (idx_bound id2 idx2_pre fvs2_pre depth
              (Imply(Eq(Id idx2, Id idx2_pre), sl2)))) in
    let idx1_pre = make_idx_id fun_num id1 branch_trace (depth+1) in
    let fvs1_pre = idx1_pre::fvs in
    let idx2 = make_idx_id fun_num id2 branch_trace (depth+1) in
    let fvs2 = idx2::fvs in
    let idx2_pre = make_idx_id fun_num id2 branch_trace (depth+1) in
    let fvs2_pre = idx2_pre::fvs in
    And(common depth,
      idx_bound id1 idx1_pre fvs (depth+1)
      ...
      let rec same_range_post fvs2 fvs2_pre depth =
        if depth <= 0 then True
          ...
          And(sl1, 
           ...
           (idx_bound id2 idx2_pre fvs depth
            (Imply(Eq(Id idx2, Id idx2_pre), sl2)))) in
        let idx2 = make_idx_id fun_num id2 branch_trace (depth+1) in
        let fvs2 = idx2::fvs in
        let idx2_pre = make_pre_idx_id fun_num id2 branch_trace (depth+1) in
        ...
        And(common depth, 
         idx_bound id2 idx2 fvs (depth+1)
         (idx_bound id2 idx2_pre fvs (depth+1)
          (Imply(Eq(Id idx2, Id idx2_pre), same_range_post fvs2 fvs2_pre depth)))) in
  ...
  | CLetDeref (id1, id2, pos) -> 
    ...
    sl1 @ 
    List.map
      (fun x -> Imply(make_idx_bound_smtlib id2 idx2 fvs fun_num branch_trace id2_depth, x))
      sl2
    (* [] *)
