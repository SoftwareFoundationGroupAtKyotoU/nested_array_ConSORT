...
let make_pre_idx_id fun_num id branch_trace depth = 
  let id_pos = lookup_pre_pos id branch_trace !var_locations in
  asprintf "i_%d_%s_%d_%dth%a" fun_num id id_pos depth pp_branch_trace branch_trace
...
let rec same_own id fvs fun_num branch_trace depth =
  if depth <= 0 then []
  else 
...
let rec make_if_smtlib id fvs fvs_then fvs_else fun_num branch_trace depth =
  if depth <= 0 then []
  else
    ...
    Eq(make_bound_exp fvs id "l" fun_num branch_trace depth, make_bound_exp fvs_then id "l" fun_num (Then :: branch_trace) depth);
    Eq(make_bound_exp fvs id "l" fun_num branch_trace depth, make_bound_exp fvs_else id "l" fun_num (Else :: branch_trace) depth);
    Eq(make_bound_exp fvs id "h" fun_num branch_trace depth, make_bound_exp fvs_then id "h" fun_num (Then :: branch_trace) depth);
    Eq(make_bound_exp fvs id "h" fun_num branch_trace depth, make_bound_exp fvs_else id "h" fun_num (Else :: branch_trace) depth)] in
    let idx = make_idx_id fun_num id branch_trace depth in
    let fvs' = idx::fvs in
    let idx_then = make_idx_id fun_num id (Then::branch_trace) depth in
    let fvs'_then = idx_then::fvs_then in
    let idx_else = make_idx_id fun_num id (Else::branch_trace) depth in
    let fvs'_else = idx_else::fvs_else in
    (* 添え字のたどり方が等しいならば所有権の値は等しい，の添え字のたどり方が等しいならばの部分 *)
    let idx_equality x = Imply(And(Eq(Id idx, Id idx_then), Eq(Id idx, Id idx_else)), x) in 
    (* 添え字が配列の境界内にあるという制約 *)
    let idx_bound fvs branch_trace x = Imply(make_idx_bound_smtlib id fvs fun_num branch_trace depth, x) in
    let sl2 = 
      List.map
        (fun x -> idx_bound fvs_else (Else :: branch_trace) (idx_bound fvs_then (Then :: branch_trace) (idx_bound fvs branch_trace x)))
        (List.map 
          idx_equality 
          (make_if_smtlib id fvs' fvs'_then fvs'_else fun_num branch_trace (depth-1))) in
    sl @ sl2

let make_post_if_smtlib fvs id fun_num branch_trace depth branch =
  let branch_trace' = branch :: branch_trace in
  let rec make_post_if_smtlib_sub fvs fvs_branch depth =
    if depth <= 0 then
      True
    else
      let idx = make_idx_id fun_num id branch_trace depth in
      let fvs' = idx::fvs in
      let idx_branch = make_idx_id fun_num id branch_trace' depth in
      let fvs'_branch = idx_branch::fvs_branch in
      let idx_equality x = Imply(Eq(Id idx, Id idx_branch), x) in 
        (* 添え字が配列の境界内にあるという制約 *)
      let idx_bound fvs branch_trace = And(make_idx_bound_smtlib id fvs fun_num branch_trace depth, make_idx_bound_smtlib id fvs fun_num branch_trace depth) in
      let sl = make_post_if_smtlib_sub fvs' fvs'_branch (depth - 1) in
      let sl2 = And(Leq(make_own_var id fun_num branch_trace depth, make_own_var id fun_num branch_trace' depth),
        And(Geq(make_bound_exp fvs id "l" fun_num branch_trace depth, make_bound_exp fvs_branch id "l" fun_num branch_trace' depth),
        And(Leq(make_bound_exp fvs id "h" fun_num branch_trace depth, make_bound_exp fvs_branch id "h" fun_num branch_trace' depth), 
        Imply(And(idx_bound fvs branch_trace, idx_bound fvs_branch branch_trace'),
        idx_equality sl)))) in
      sl2 in
  ...
  make_post_if_smtlib_sub fvs fvs depth))]
...
let make_letAddPtr_smtlib_heuristic fvs fun_num branch_trace id1 id2 depth =
  ...
  let rec make_letAppPtr_smtlib_range fvs1 fvs2 fvs2_pre depth =
    if depth <= 0 then True
      ...
      let idx_bound id fvs = make_idx_bound_smtlib id fvs fun_num branch_trace depth in
      let sl1 = 
      ...
      And(sl1, 
        Imply(And(idx_bound idx1 fvs1, And(idx_bound idx2 fvs2, idx_bound idx2_pre fvs2_pre)),
  ...
  let idx_bound id fvs = make_idx_bound_smtlib id fvs fun_num branch_trace depth in
  [And(sl1, sl2);
  Imply(And(idx_bound idx1 fvs, And(idx_bound idx2 fvs, idx_bound idx2_pre fvs)),
...
let make_assignRef_smtlib fvs fun_num branch_trace id1 id2 depth =
  ...
  let idx_bound id fvs depth x = 
    Imply(make_idx_bound_smtlib id fvs fun_num branch_trace depth,
    x) in
  ...
  let rec first_element fvs1_0 fvs2 depth =
    ...
    if depth <= 0 then []
      ...
      List.map 
        (fun x -> 
          idx_bound idx2 fvs2 depth (idx_bound idx1_0 fvs1_0 depth x))
          (List.map 
            (fun x -> Imply(Eq(Id idx1_0, Id idx2), x)) 
  ...
  let rec not_first_element fvs1 fvs1_non0 depth =
    ...
    if depth <= 0 then []
      ...
      List.map 
        (fun x -> 
          idx_bound idx1_non0 fvs1_non0 depth (idx_bound idx1 fvs1 depth x))
          (List.map 
            (fun x -> Imply(Eq(Id idx1, Id idx1_non0), x)) 
...
let make_letDeref_smtlib fvs1 fvs2 fun_num branch_trace id1 id2 depth =
  ...
  let rec make_letDeref_smtlib_same_range id1 id2 fvs1 fvs2 depth =
    if depth <= 0 then []
      ...
      sl1 @ 
      List.map 
      (fun x -> idx_bound idx1 fvs1 depth (idx_bound idx2 fvs2 depth x))
      (List.map 
        (fun x -> Imply(Eq(Id idx1, Id idx2), x)) 
        sl2) in
  let outer_idx2 = make_pre_idx_id fun_num id2 branch_trace (depth+1) in
  common depth @
  ...
  List.map (fun x -> Imply(Not(Eq(Id outer_idx2, Id "0")), x)) (make_letDeref_smtlib_same_range id2_non0 id2 fvs2 fvs2 depth)

let rec alias_to_alias fvs fun_num branch_trace id1 id2 depth =
  if depth <= 0 then True
  else
    let id1_pre_own = make_pre_own_var id1 fun_num branch_trace depth in
    let id1_pre_scope_low = make_pre_bound_exp fvs id1 "l" fun_num branch_trace depth in
    let id1_pre_scope_high = make_pre_bound_exp fvs id1 "h" fun_num branch_trace depth in
    let id2_pre_own = make_pre_own_var id2 fun_num branch_trace depth in
    let id2_pre_scope_low = make_pre_bound_exp fvs id2 "l" fun_num branch_trace depth in
    let id2_pre_scope_high = make_pre_bound_exp fvs id2 "h" fun_num branch_trace depth  in
    let id1_post_own = make_own_var id1 fun_num branch_trace depth in
    let id1_post_scope_low = make_bound_exp fvs id1 "l" fun_num branch_trace depth in
    let id1_post_scope_high = make_bound_exp fvs id1 "h" fun_num branch_trace depth in
    let id2_post_own = make_own_var id2 fun_num branch_trace depth in
    let id2_post_scope_low = make_bound_exp fvs id2 "l" fun_num branch_trace depth in
    let id2_post_scope_high = make_bound_exp fvs id2 "h" fun_num branch_trace depth in
    ...
    let smtlib4 = 
      ...
      And(Eq(id1_post_own, id2_post_own),
          ...
          Or(And(Eq(id2_pre_scope_low, id1_post_scope_low), 
              ...
              And(adjacent_id2_pre_hi_id1_pre_lo,
                adjacent_id2_post_hi_id1_post_lo))))))))) in
    And(Or(And(smtlib1, alias_to_alias fvs fun_num branch_trace id1 id2 (depth-1) ),
      Or(And(smtlib2, alias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1) ),
      Or(And(smtlib3, nonalias_to_alias fvs fun_num branch_trace id1 id2 (depth-1) ),
        And(smtlib4, nonalias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1) )))),
    And(Leq(make_bound_exp fvs id1 "l" fun_num branch_trace depth, make_bound_exp fvs id1 "h" fun_num branch_trace depth),
        Leq(make_bound_exp fvs id2 "l" fun_num branch_trace depth, make_bound_exp fvs id2 "h" fun_num branch_trace depth)))
and alias_to_nonalias fvs fun_num branch_trace id1 id2 depth =
  if depth <= 0 then True
  else
    let id1_pre_own = make_pre_own_var id1 fun_num branch_trace depth in
    let id1_pre_scope_low = make_pre_bound_exp fvs id1 "l" fun_num branch_trace depth in
    ...
    let smtlib2 =
      And(Eq(id1_pre_own, id1_post_own),
      And(Eq(id2_pre_own, id2_post_own),
      And(Eq(id1_pre_own, id2_pre_own),
      And(Eq(id1_post_scope_low, id2_post_scope_low),
      And(Eq(id1_post_scope_high, id2_post_scope_high),
          Or(And(Eq(id1_pre_scope_low, id1_post_scope_low), 
              And(Eq(id2_pre_scope_high, id2_post_scope_high),
              ...
              And(Eq(id1_pre_scope_high, id2_post_scope_high),
                adjacent_id2_pre_hi_id1_pre_lo)))))))) in
    And(Or(And(smtlib1, alias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1) ),
           And(smtlib2, nonalias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1) )),
    And(Leq(make_bound_exp fvs id1 "l" fun_num branch_trace depth, make_bound_exp fvs id1 "h" fun_num branch_trace depth),
        Leq(make_bound_exp fvs id2 "l" fun_num branch_trace depth, make_bound_exp fvs id2 "h" fun_num branch_trace depth)))
and nonalias_to_alias fvs fun_num branch_trace id1 id2 depth =
  if depth <= 0 then True
  else
    let id1_pre_own = make_pre_own_var id1 fun_num branch_trace depth in
    let id1_pre_scope_low = make_pre_bound_exp fvs id1 "l" fun_num branch_trace depth in
    ...
    let smtlib2 = 
      ...
      And(Eq(id1_pre_scope_high, id1_post_scope_high),
      Eq(id2_pre_scope_high, id2_post_scope_high)))))))) in
    And(Or(And(smtlib1, nonalias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1) ),
           And(smtlib2, nonalias_to_alias fvs fun_num branch_trace id1 id2 (depth-1) )),
    And(Leq(make_bound_exp fvs id1 "l" fun_num branch_trace depth, make_bound_exp fvs id1 "h" fun_num branch_trace depth),
        Leq(make_bound_exp fvs id2 "l" fun_num branch_trace depth, make_bound_exp fvs id2 "h" fun_num branch_trace depth)))
and nonalias_to_nonalias fvs fun_num branch_trace id1 id2 depth =
  if depth <= 0 then True
  else
    let id1_pre_own = make_pre_own_var id1 fun_num branch_trace depth in
    let id1_pre_scope_low = make_pre_bound_exp fvs id1 "l" fun_num branch_trace depth in
    ...
    let smtlib1 =
      ...
      Eq(id2_pre_scope_high, id2_post_scope_high))))))))) in
    And(smtlib1,
    And(nonalias_to_nonalias fvs fun_num branch_trace id1 id2 (depth-1),
    And(Leq(make_bound_exp fvs id1 "l" fun_num branch_trace depth, make_bound_exp fvs id1 "h" fun_num branch_trace depth),
        Leq(make_bound_exp fvs id2 "l" fun_num branch_trace depth, make_bound_exp fvs id2 "h" fun_num branch_trace depth))))
...
let make_aliasDeref_smtlib fvs fvs_pre fun_num branch_trace id1 id2 depth =
  ...
  let rec common depth =
    if depth <= 0 then []
      ...
      [Eq(make_pre_own_var id1 fun_num branch_trace depth, make_own_var id2 fun_num branch_trace depth);
      Eq(make_own_var id1 fun_num branch_trace depth, Id "0.");
      Eq(make_pre_own_var id2_0 fun_num branch_trace depth, Id "0.")]
      @ (common (depth-1)) in
  let rec make_aliasDeref_smtlib_same_range_pre fvs1 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      let sl1 = 
        ...
        Eq(make_pre_bound_exp fvs2_non0 id2_non0 "h" fun_num branch_trace depth, make_pre_bound_exp fvs1 id1 "h" fun_num branch_trace depth);
        ] in
      let sl2 = make_aliasDeref_smtlib_same_range_pre fvs1' fvs2'_non0 (depth-1) in
      sl1 @ 
      List.map
        (fun x -> And(idx_bound id1 fvs1 depth x, idx_bound id2_non0 fvs2_non0 depth x)) 
        (List.map (fun x -> Imply(Eq(Id idx1, Id idx2_non0), x)) sl2) in
  let rec make_aliasDeref_smtlib_same_range_post fvs2 fvs2_non0 depth =
    if depth <= 0 then []
      ...
      sl1 @ 
      List.map
        (fun x -> idx_bound id2 fvs2 depth (idx_bound id2_non0 fvs2_non0 depth x))
        (List.map (fun x -> Imply(Eq(Id idx2, Id idx2_non0), x)) sl2) in
    let idx2_pre = make_pre_idx_id fun_num id2 branch_trace (depth+1) in
    common depth 
    @ List.map 
      (fun x -> Imply(Eq(Id "0", Id idx2_pre), x)) 
      (make_aliasDeref_smtlib_same_range_pre fvs_pre fvs_pre depth) 
    @ List.map 
      (fun x -> Imply(Not(Eq(Id "0", Id idx2_pre)), x)) 
      (make_aliasDeref_smtlib_same_range_post fvs fvs_pre depth)
...
let rec constr_to_smtlib fvs fun_num funnames_numberings branch_trace ty_env c =
      ...
      List.concat (List.map 
        (fun id -> 
          ...
          (* if式の制約
           ...
           make_if_smtlib id fvs fvs fvs fun_num branch_trace depth
        ) ids_pre) in
  ...
  | CAssignRef (id1, id2, pos) -> 
    ...
    sl1 @ 
    List.map
      (fun x -> Imply(make_idx_bound_smtlib idx1 fvs fun_num branch_trace id1_depth, x)) 
      sl2
  | CAliasAddPtr (id1,id2,e,pos) -> (* alias(id1 = id2 + num); ... *)
    ...
    let make_aliasdAddPtr_smtlib_gather depth =
      ...
      let rec same_range_pre fvs1 fvs2 depth =
        if depth <= 0 then True
          ...
          And(sl1, 
            Imply(Eq(Id idx1, Id idx2), sl2)) in
      let rec same_range_post fvs2 fvs2_pre depth =
        if depth <= 0 then True
        else
          let idx2 = make_idx_id fun_num id2 branch_trace depth in
          let fvs2' = idx2::fvs2 in
          let idx2_pre = make_idx_id fun_num id2 branch_trace depth in
          let fvs2'_pre = idx2_pre::fvs2_pre in
          let sl1 =
            (* id2の所有範囲は変化しない *)
          And(Eq(make_pre_bound_exp fvs2_pre id2 "l" fun_num branch_trace depth,
          ...
          Eq(make_pre_bound_exp fvs2_pre id2 "h" fun_num branch_trace depth,
            make_bound_exp fvs2 id2 "h" fun_num branch_trace depth)) in
          let sl2 = same_range_post fvs2' fvs2'_pre (depth - 1) in
          And(sl1, Imply(Eq(Id idx2, Id idx2_pre), sl2)) in
    let idx1_pre = make_idx_id fun_num id1 branch_trace depth in
    ...
    let make_aliasdAddPtr_smtlib_no_change depth =
      ...
      let rec same_range_post fvs2 fvs2_pre depth =
        ...
        let idx2_pre = make_pre_idx_id fun_num id2 branch_trace depth in
        let fvs2_pre = idx2_pre::fvs in
        And(common depth, same_range_post fvs2 fvs2_pre depth) in
  ...
  | CLetDeref (id1, id2, pos) -> 
    ...
    new_id id1 pos branch_trace id1_simpleTy; 
    new_id id2 pos branch_trace id2_simpleTy;
    new_id id2_0 pos branch_trace id1_simpleTy;
    new_id id2_non0 pos branch_trace id1_simpleTy;
    let id1_depth = ref_depth id1_simpleTy in
    let id2_depth = ref_depth id2_simpleTy in
    ...
    let sl1 = [Gt(make_pre_own_var id2 fun_num branch_trace id2_depth, Id "0");
     ...
     Eq(make_pre_own_var id2 fun_num branch_trace id2_depth, make_own_var id2 fun_num branch_trace id2_depth);
     Eq(make_pre_bound_exp fvs id2 "l" fun_num branch_trace id2_depth, 
        make_bound_exp fvs id2 "l" fun_num branch_trace id2_depth);
     Eq(make_pre_bound_exp fvs id2 "h" fun_num branch_trace id2_depth, 
        make_bound_exp fvs id2 "h" fun_num branch_trace id2_depth)] in
    let fvs1 = fvs in
    let idx2 = make_pre_idx_id fun_num id2 branch_trace id2_depth in
    let fvs2 = idx2::fvs in
     let sl2 = make_letDeref_smtlib fvs1 fvs2 fun_num branch_trace id1 id2 id1_depth in
    sl1 @ 
    List.map
      (fun x -> Imply(make_idx_bound_smtlib id2 fvs fun_num branch_trace id2_depth, x))
      sl2
    (* [] *)
  ...
  | CAliasDeref (id1, id2, pos) -> 
      ...
      let id2_pre_own = make_pre_own_var id2 fun_num branch_trace id2_depth in
      let id2_pre_scope_low = make_pre_bound_exp fvs id2 "l" fun_num branch_trace id2_depth in
      let id2_pre_scope_high = make_pre_bound_exp fvs id2 "h" fun_num branch_trace id2_depth  in
...
let fun_constrs_to_smtlib funname_constrs fun_num funnames_numberings =
  ...
  let ref_id_before_eval_to_smtlibs id = 
    ...
    | ENull ->
      ...
      [Eq(make_own_var id fun_num [] depth, make_own_var_be id fun_num "b" depth);
       Eq(make_bound_exp fvs id "l" fun_num [] depth, make_bound_exp_be fvs_b id "l" fun_num "b" depth);
       Eq(make_bound_exp fvs id "h" fun_num [] depth, make_bound_exp_be fvs_b id "h" fun_num "b" depth);
  ...
  in ref_id_before_eval_to_smtlibs_sub ftype fvs fvs in
  ...
  let ref_id_after_eval_to_smtlibs id = 
    ...
    | ENull ->
      ...
      [Or(Eq(make_own_var_be id fun_num "e" depth, Id "0."),
       ...
       And(Geq(make_bound_exp_be fvs_e id "l" fun_num "e" depth, make_bound_exp fvs id "l" fun_num [] depth),
  ...
  in ref_id_after_eval_to_smtlibs_sub ftype fvs fvs in
